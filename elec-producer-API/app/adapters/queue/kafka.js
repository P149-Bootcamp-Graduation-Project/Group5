  ...
  try {
    ...
    producer.on("ready", async function () {
      let push_status = producer.send(payload, (err, data) => {
        if (err) {
          console.log(
            "kafka-producer -> " + kafka_topic + " kafka topic error" + err
          );
        } else {
          console.log(
            "kafka-producer -> " + kafka_topic + " kafka topic success"
          );
        }
      });
    });
    producer.on("error", function (err) {
      console.log(err);
      console.log(
        "kafka-producer -> " + kafka_topic + " kafka connection error"
      );
      throw err;
    });
